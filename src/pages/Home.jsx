...
import { 
  ...
  Timer, Loader2, History, AlertTriangle, Check, Share2 
} from 'lucide-react';
...
const Home = () => {
  ...
  const handlePaymentReturn = async (paymentId, owned, stateParam) => {
    ...
    if (foundLocal) foundToken = foundLocal.token;
      ...
      try {
        ...
        if (decoded.t && decoded.id === paymentId) {
          ...
          if (!stored.find(i => i.id === paymentId)) {
             const newEntry = { id: paymentId, token: foundToken, createdAt: new Date().toISOString(), ...recoveredData };
    ...
    if (foundToken) {
      ...
      const optimisticInvite = {
        id: paymentId,
        sender: recoveredData?.sender || foundLocal?.sender || "Vous...",
        valentine: recoveredData?.valentine || foundLocal?.valentine || "...",
        plan: 'spy'
      };
  ...
  const waitForServerValidation = async (paymentId, foundLocal) => {
    ...
    const poll = async () => {
      ...
      if (isPaid) {
        ...
        delay = Math.min(delay * 1.2, 4000); // Backoff
  ...
  const displaySuccess = (invite, token) => {
    ...
    setFormData({ sender: invite.sender, valentine: invite.valentine, plan: invite.plan });
  ...
  const handleShare = async (text, field) => {
    if (navigator.share && navigator.canShare) {
      try {
        ...
        if (err.name !== 'AbortError') copyToClipboard(text, field);
      }
    } else {
    ...
    <div className="min-h-screen p-4 flex flex-col items-center justify-center relative overflow-x-hidden pt-16">
      ...
      <header className="text-center mb-10 relative z-10">
        ...
        <p className="text-rose-gold text-lg font-serif italic tracking-wider">
          L'élégance d'une demande irréfusable.
        </p>
      </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <div className="bg-orange-500/10 border border-orange-500/30 p-4 rounded-lg flex items-start gap-3">
            ...
            <p className="text-xs text-orange-200/80 leading-relaxed">
             <strong>Important :</strong> Ne changez pas d'appareil ou de navigateur pendant le paiement. Votre clé secrète pour le Dashboard Espion sera générée sur cet appareil.
            </p>
          </div>
          ...
          >
            {status === 'idle' && (
              <span className="relative z-10 flex items-center justify-center gap-3">
                Sceller l'invitation <Heart size={16} fill="currentColor" className="animate-pulse-slow" />
              </span>
            )}
            {status === 'processing' && (
      ...
      <footer className="w-full max-w-2xl text-center pb-8 opacity-50 hover:opacity-100 transition-opacity z-10">
        <p className="text-[10px] text-cream uppercase tracking-widest mb-2">© {new Date().getFullYear()} YesOrYes. Tous droits réservés.</p>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <Heart className="absolute bottom-40 right-1/4 text-rose-pale opacity-20 w-12 h-12 animate-float delay-1000 blur-[1px]" />
      </div>

      {/* Notifications Sociales Fake */}
      {activeNotif && (
